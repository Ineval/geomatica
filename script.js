const SUPABASE_URL = "https://apiqfherndzzulzwrryq.supabase.co";
    ...
    function refreshVariableCheckboxes() {
      ...
      if (allowed.has('general')) addSubjectCheckbox('general');

      const subjectItems = orderedSubjects.filter(k => k !== 'general');
      if (subjectItems.length) {
        container.insertAdjacentHTML('beforeend', `
          <h2 class="tool-title" style="margin-top:8px;">
            <svg viewBox="0 0 24 24"><path d="M4 6H2v14c0 1.1.9 2 2 2h14v-2H4V6zm16-4H8c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm0 14H8V4h12v12z"/></svg>
            Asignaturas
          </h2>
        `);
        subjectItems.forEach(addSubjectCheckbox);
      }
    ...
    function updateStats() {
      ...
      selectedLvl.forEach(l => {
        ...
        selectedVars.forEach(v => {
          ...
          levelHtml += `<div class="subject-row"><span>${variables[v].label}</span><b>${st.avg}</b><span>${st.min}</span><span>${st.max}</span></div><div class="stat-count">${st.count} "I.E"</div>`;
        });
